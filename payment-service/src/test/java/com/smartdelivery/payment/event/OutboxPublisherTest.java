...
class OutboxPublisherTest {
    ...
    private OutboxEvent pendingEvent(UUID aggregateId, String payload) {
        OutboxEvent event = new OutboxEvent("Payment", aggregateId, "PaymentCompleted", KafkaTopics.PAYMENT_COMPLETED, payload);
    ...
    void publishPendingMarksARowPublishedOnASuccessfulSend() {
        ...
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISHED, "eventType", "PaymentCompleted").count()).isEqualTo(1);
    }
    ...
    void publishPendingLeavesARowPendingAndRecordsTheErrorWhenTheSendFails() {
        ...
        verify(outboxEventRepository).save(event);
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISH_FAILURES, "eventType", "PaymentCompleted").count())
                .isEqualTo(1);
    }
