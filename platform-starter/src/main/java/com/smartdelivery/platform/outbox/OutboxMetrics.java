 ...
 * is the question that actually matters and the one nothing could answer before
 * Phase 15. A non-zero pending count is normal -- rows land continuously and drain
 * within a poll interval -- so the alertable signal is a pending count that only grows,
 * or an oldest-pending age well past the poll interval. Either means events are
 * committed but stuck, which downstream looks exactly like a silently stalled saga.
 */
public class OutboxMetrics {

    public static final String PENDING_COUNT = "outbox.pending.count";
    public static final String OLDEST_PENDING_AGE = "outbox.oldest.pending.age.seconds";
    public static final String PUBLISHED = "outbox.published";
    public static final String PUBLISH_FAILURES = "outbox.publish.failures";
    public static final String CLEANUP_DELETED = "outbox.cleanup.deleted";
