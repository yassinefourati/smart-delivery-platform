...
class OutboxPublisherTest {
    ...
    /** A stand-in for whichever topic a service publishes to; the poller never reads it. */
    private static final String TOPIC = "aggregate.something-happened";
    ...
    private OutboxEvent pendingEvent(UUID aggregateId, String payload) {
        OutboxEvent event = new OutboxEvent("Aggregate", aggregateId, "SomethingHappened", TOPIC, payload);
    ...
    void publishPendingMarksARowPublishedOnASuccessfulSend() {
        ...
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISHED, "eventType", "SomethingHappened").count()).isEqualTo(1);
    }
    ...
    void publishPendingLeavesARowPendingAndRecordsTheErrorWhenTheSendFails() {
        ...
        verify(outboxEventRepository).save(event);
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISH_FAILURES, "eventType", "SomethingHappened").count())
                .isEqualTo(1);
    }
