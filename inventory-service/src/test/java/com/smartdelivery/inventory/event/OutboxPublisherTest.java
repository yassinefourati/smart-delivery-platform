...
class OutboxPublisherTest {
    ...
    private OutboxEvent pendingEvent(UUID aggregateId, String payload) {
        OutboxEvent event = new OutboxEvent("InventoryReservation", aggregateId, "InventoryReserved", KafkaTopics.INVENTORY_RESERVED, payload);
    ...
    void publishPendingMarksARowPublishedOnASuccessfulSend() {
        ...
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISHED, "eventType", "InventoryReserved").count()).isEqualTo(1);
    }
    ...
    void publishPendingLeavesARowPendingAndRecordsTheErrorWhenTheSendFails() {
        ...
        verify(outboxEventRepository).save(event);
        assertThat(meterRegistry.counter(OutboxMetrics.PUBLISH_FAILURES, "eventType", "InventoryReserved").count())
                .isEqualTo(1);
    }
